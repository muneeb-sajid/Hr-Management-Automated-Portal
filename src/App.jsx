...
const Dashboard = () => {
  ...
  const columns = useMemo(
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center space-x-3 min-w-[180px]">
              <div
                className={`w-9 h-9 rounded-full flex items-center justify-center font-medium text-xs shrink-0 ${row.original.avatarBg}`}
              >
                {initials}
              </div>
              <div className="truncate">
                ...
                <p className="text-xs text-slate-500 dark:text-gray-400 truncate">{row.original.role}</p>
              </div>
            </div>
          );
        },
      },
      {
        header: 'Action',
        accessorKey: 'action',
        cell: (info) => <span className="text-xs sm:text-sm text-slate-700 dark:text-gray-300 min-w-[140px] block">{info.getValue()}</span>,
      },
      {
        header: 'Date',
        accessorKey: 'date',
        cell: (info) => <span className="text-xs sm:text-sm text-slate-600 dark:text-gray-400 whitespace-nowrap">{info.getValue()}</span>,
      },
      {
        header: 'Status',
        accessorKey: 'status',
        cell: ({ getValue, row }) => (
          <span
            className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold border whitespace-nowrap ${row.original.statusBadge}`}
          >
            {getValue()}
          </span>
        ),
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 bg-[#f9f9ff] dark:bg-gray-950 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          <button className="flex-1 sm:flex-none flex items-center justify-center space-x-2 px-3.5 py-2 bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl text-xs sm:text-sm font-medium text-slate-700 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-700 transition-colors shadow-xs">
            <Download className="w-4 h-4 text-slate-500 dark:text-gray-400" />
            <span>Export CSV</span>
          </button>
          <button 
            className="flex-1 sm:flex-none flex items-center justify-center space-x-2 px-3.5 py-2 bg-indigo-600 dark:bg-indigo-500 rounded-xl text-xs sm:text-sm font-medium text-white hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors shadow-xs" 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {kpiCards.map((card, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-white dark:bg-gray-900 p-5 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-gray-800 shadow-xs flex flex-col justify-between"
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] sm:text-xs font-semibold text-slate-400 dark:text-gray-500 tracking-wider">
                  {card.title}
                </span>
              ...
              <div className="mt-3 sm:mt-4">
                ...
                <div className="flex items-center space-x-2 mt-2">
                  <span className={`text-[11px] px-2 py-0.5 rounded-md ${card.badgeBg}`}>
                    {card.badgeText}
                  </span>
                  <span className="text-[11px] text-slate-400 dark:text-gray-500">{card.subtext}</span>
                </div>
              </div>
            </div>
          );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-gray-900 p-5 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-gray-800 shadow-xs flex flex-col justify-between space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h2 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Attendance Trends</h2>
              <p className="text-xs text-slate-500 dark:text-gray-400">Workforce stability over the last 30 days</p>
            </div>
            <select className="self-start sm:self-auto text-xs font-medium bg-[#f9f9ff] dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-700 dark:text-gray-300 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
        ...
        <div className="lg:col-span-4 bg-white dark:bg-gray-900 p-5 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-gray-800 shadow-xs flex flex-col justify-between space-y-4">
          <div>
            <h2 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Department Headcount</h2>
          ...
          <div className="relative h-40 sm:h-48 my-1 flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={departmentData}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white leading-tight">1,248</span>
              <span className="text-[10px] sm:text-[11px] font-medium text-slate-400 dark:text-gray-500">Total</span>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-slate-200/80 dark:border-gray-800 shadow-xs overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-gray-800 flex items-center justify-between">
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id} className="border-b border-slate-100 dark:border-gray-800 bg-[#f9f9ff] dark:bg-gray-800/50">
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className="px-4 sm:px-6 py-3 text-[11px] sm:text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wider"
                    >
              ...
              {table.getRowModel().rows.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50/60 dark:hover:bg-gray-800/60 transition-colors">
                  {row.getVisibleCells().map((cell) => (
                    <td key={cell.id} className="px-4 sm:px-6 py-3.5">
